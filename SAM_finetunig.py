...
from dataset.dataset_ACDC import BaseDataSets_SAM,RandomGenerator,BaseDataSets111,RandomGenerator111
...
def get_args_parser():
    ...
    parser.add_argument('--root_path', type=str, default='/home/cj/code/SAM_Scribble/data/ACDC')
    ...
    parser.add_argument('--sam_checkpoint', type=str, default='model_path/sam_vit_h_4b8939.pth', help='SAM model checkpoint')
...
def main(opts, snapshot_path, savepath):
    ...
    train_set = BaseDataSets_SAM(base_dir=opts.root_path, fold='fold1',transform=transforms.Compose(
        [RandomGenerator(opts.patch_size,split='train')]), split="train",pesudo_label = 'SAM_iteration1')
    

    val_set = BaseDataSets_SAM(base_dir=opts.root_path, fold='fold1', split="val", transform=transforms.Compose(
        [RandomGenerator(opts.patch_size,split='val')]),pesudo_label = 'SAM_iteration1')
    ...
    for epoch in range(opts.max_epochs):
        ...
        val_dice_loss, val_iou_loss, val_dice, val_HD95, val_iou = SAM_trainer.model_evaluate(
            model=sam,
        ...
        save_best_path_all_perepoch = os.path.join(savepath,f'sam_all_{val_dice}_{epoch}.pth')
        ...
        print(f'val_dice_loss: {val_dice_loss:.5f}, val_iou_loss: {val_iou_loss:.5f}, val_dice: {val_dice:.5f}, val_iou: {val_iou:.5f}, val_HD95:{val_HD95:.5f} \n')
