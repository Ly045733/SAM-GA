...
class UNet_CCT_SlotAttention(nn.Module):
    def __init__(self, in_chns, class_num, channel=128):
        super(UNet_CCT_SlotAttention, self).__init__()

        params = {'in_chns': in_chns,
                  'feature_chns': [64, 256, 512, 1024, 2048],
                  'dropout': [0.05, 0.1, 0.2, 0.3, 0.5],
                  'class_num': class_num,
                  'bilinear': False,
                  'acti_func': 'relu'}
        ...
        self.main_decoder = Decoder(params)
        self.aux_decoder1 = Decoder(params)
        self.gcm_interFA = GCM_interFA_ODE_slot_channel4_scribble(256, channel, params)
        self.GPM = GPM()
...
class UNet_CCT_SlotAttention_thin(nn.Module):
    def __init__(self, in_chns, class_num, channel=128):
        super(UNet_CCT_SlotAttention_thin, self).__init__()
...
if __name__ == '__main__':
    ...
    model = UNet_CCT_SlotAttention_thin_aux(3,4,channel = 64).cuda()
    main_seg, aux_seg1,out = model(image)
